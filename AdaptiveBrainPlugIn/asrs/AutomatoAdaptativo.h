...
#define QUERY_TBL_PLANO       "CREATE TABLE Plano(IdRegraPlano INTEGER NOT NULL, IdEstadoInicial INTEGER NOT NULL,IdCondicao VARCHAR(2) NOT NULL,IdEstadoFinal INTEGER NOT NULL,Tipo CHAR(1) NOT NULL, PRIMARY KEY (IdRegraPlano) FOREIGN KEY (IdCondicao) REFERENCES Condicoes (CondId) FOREIGN KEY (IdEstadoFinal) REFERENCES Estados (EstadoId) FOREIGN KEY (IdEstadoInicial) REFERENCES Estados (EstadoId));" //#ECa#Campo RobotPlan transferido para a tabela de Estados.
#define QUERY_TBL_ESTADOS     "CREATE TABLE Estados(EstadoId INTEGER NOT NULL,RobotPlan INTEGER NOT NULL,	PRIMARY KEY (EstadoId));" //#ECa#Campo RobotPlan transferido para a tabela de Estados.
...
#define QUERY_TRANSITAR                 "SELECT Plano.IdEstadoFinal AS EstadoFinal, Estados.RobotPlan AS RobotPlan FROM Plano INNER JOIN Automato ON Automato.IdRegraPlano=Plano.IdRegraPlano INNER JOIN Condicoes ON Plano.IdCondicao=Condicoes.CondId INNER JOIN Estados ON Plano.IdEstadoFinal=Estados.EstadoId WHERE "
...
#define CONDICAO_PARA_ESTADO_INICIAL    "0000000000000000"
